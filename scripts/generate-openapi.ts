 ...
 *
 * Run from the payment-bff directory: npm run generate:openapi
 *
...
async function generate(): Promise<void> {
  ...
  const config = new DocumentBuilder()
    .setTitle('Payment BFF API')
    .setDescription('Thin REST aggregation BFF over payment-service. Pass-through + response shaping; holds no domain logic and no datastore.')
    .setVersion('0.1.0')
    .build();
